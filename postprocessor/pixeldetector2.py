# This Pixel Detector keeps image's aspect ratio
...
def pixel_detect(image: Image):
    ...
    # Compute a single scale factor
    scale = max(np.median(hspacing), np.median(vspacing))

    # Resize input image using kCentroid with the calculated scale factor
    return kCentroid(image, round(image.width/scale), round(image.height/scale), 2), scale
...
if os.path.isfile(args["input"]):
    ...
    downscale, scale = pixel_detect(image)
